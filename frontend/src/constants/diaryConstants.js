export const DIARY_LIST_REQUEST = 'DIARY_LIST_REQUEST';
export const DIARY_LIST_SUCCESS = 'DIARY_LIST_SUCCESS';
export const DIARY_LIST_FAIL = 'DIARY_LIST_FAIL';

export const DIARY_DETAILS_REQUEST = 'DIARY_DETAILS_REQUEST';
export const DIARY_DETAILS_SUCCESS = 'DIARY_DETAILS_SUCCESS';
export const DIARY_DETAILS_FAIL = 'DIARY_DETAILS_FAIL';

export const ADD_NEW_DIARY_REQUEST = 'ADD_NEW_DIARY_REQUEST';
export const ADD_NEW_DIARY_SUCCESS = 'ADD_NEW_DIARY_SUCCESS';
export const ADD_NEW_DIARY_FAIL = 'ADD_NEW_DIARY_FAIL';

export const EIDT_DIARY_REQUEST = 'EIDT_DIARY_REQUEST';
export const EIDT_DIARY_SUCCESS = 'EIDT_DIARY_SUCCESS';
export const EIDT_DIARY_FAIL = 'EIDT_DIARY_FAIL';
